...
export function createRootScene(engine: Engine): Scene {
  ...
  const interaction = new InteractionSystem(scene, player, input, prompt.context());
...
function populateInteractables(scene: Scene, ceilingLamp: SpotLight): void {
  ...
  attachInteractable(terminal, {
    ...
    activate: (ctx) => ctx.showInspectPanel("SHIFT LOG TERMINAL", TERMINAL_TEXT),
  });

  // Wall switch (press/toggle)
  ...
  // Pickup placeholder (small glowing shard on the desk)
  ...
  attachInteractable(pickup as unknown as AbstractMesh, {
    ...
    activate: (ctx) => {
      pickup.dispose();
      ctx.flashHint("Picked up data shard.");
    },
  });
}
